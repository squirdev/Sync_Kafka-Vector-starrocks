...
# - Fields: `id`, `title`, `author`, `publish_time`.

# Vector:

# - Source: `kafka_input` (reads JSON from Kafka).

# - Sink: `starrocks` (Doris sink, writes into StarRocks table `demo.articles`).

# - Optional sink: `console_output` (prints events for debugging).
...
# > Client → Kafka (`article-topic`) → Vector (`kafka_input`) → Vector Doris sink → StarRocks (`demo.articles`)
...
# Authentication: basic (user `vector_user`).
...
# There is also a `console_output` sink that prints the Kafka events as JSON to stdout for debugging.
